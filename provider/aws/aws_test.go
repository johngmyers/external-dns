...
func TestAWSAdjustEndpoints(t *testing.T) {
	...
	validateEndpoints(t, records, []*endpoint.Endpoint{
		...
		endpoint.NewEndpoint("cname-test-alias.zone-1.ext-dns-test-2.teapot.zalan.do", endpoint.RecordTypeAAAA, "alias-target.zone-2.ext-dns-test-2.teapot.zalan.do").WithProviderSpecific(providerSpecificAlias, "true").WithProviderSpecific(providerSpecificEvaluateTargetHealth, "true"),
...
func TestAWSDeleteRecords(t *testing.T) {
	...
	require.NoError(t, provider.DeleteRecords(context.Background(), []*endpoint.Endpoint{
		...
		endpoint.NewEndpoint("delete-test-aaaa-alias.zone-1.ext-dns-test-2.teapot.zalan.do", endpoint.RecordTypeAAAA, "foo.eu-central-1.elb.amazonaws.com").WithProviderSpecific(providerSpecificEvaluateTargetHealth, "true"),
